...
from sklearn.linear_model import LinearRegression # Pour la construction du modèle
...
print("="*60)
print("RAPPORT D'ÉVALUATION DU MODÈLE DE RÉGRESSION LINÉAIRE")
print("="*60)
...
# Pour la régression linéaire, nous utilisons les coefficients comme importance
feature_importance = pd.DataFrame({
    'Feature': X.columns,
    'Coefficient': model.coef_,
    'Importance': np.abs(model.coef_)  # Valeur absolue pour l'importance
})
feature_importance = feature_importance.sort_values('Importance', ascending=False)
print("Top 5 des features les plus importantes (coefficients) :")
print(feature_importance[['Feature', 'Coefficient', 'Importance']].head().to_string(index=False))
